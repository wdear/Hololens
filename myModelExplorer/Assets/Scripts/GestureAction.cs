...
public class GestureAction : MonoBehaviour
{
    ...
    void PerformManipulationUpdate(Vector3 position)
    {
        if (GestureManager.Instance.IsManipulating)
        {
            ...
            if (this.gameObject.name == "rack")
            {
                GameObject cube = GameObject.Find("inDraw1");
                cube.transform.position += moveVector;
                GameObject text= GameObject.Find("Canvas");
                text.transform.position += moveVector;
            }
            if (this.gameObject.name == "control")
            {
                GameObject cube = GameObject.Find("Canvas1");
                cube.transform.position += moveVector;
                GameObject text = GameObject.Find("Canvas2");
                text.transform.position += moveVector;
                GameObject text1 = GameObject.Find("Canvas3");
                text1.transform.position += moveVector;
            }
        }
    }
}